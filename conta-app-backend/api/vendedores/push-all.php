 ...
 * Tolerante a columnas opcionales faltantes (id_municipio, id_documento) que
 * solo existen en clientes con módulo FE v5.2 activo.
 *
 * Uso:
...
/**
 * Verifica si una columna existe en la tabla dada.
 */
function columnExists(PDO $db, string $tabla, string $col): bool {
    $stmt = $db->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS
                          WHERE TABLE_SCHEMA = DATABASE()
                            AND TABLE_NAME = ?
                            AND COLUMN_NAME = ?");
    $stmt->execute([$tabla, $col]);
...
function postBatch(string $url, string $email, string $token, array $registros, int $timeout = 60): array {
    ...
    curl_setopt_array($ch, [
        ...
        // XAMPP Windows a menudo no tiene curl.cainfo. Sin esto, cualquier
        // llamada HTTPS al Lumen falla con HTTP 0 por "SSL certificate problem".
...
try {
    ...
    // 3. CLIENTES
    // ────────────────────────────────────────────────────────────────────
    // Columnas opcionales chequeadas para no reventar en BDs viejas:
    //   • FE v5.2: id_municipio, id_documento
    //   • GPS: latitud, longitud, precision_gps_metros, gps_capturado_at
    //     (los baja pull.php cuando el vendedor crea el cliente en móvil;
    //     acá se suben de vuelta para que otros vendedores puedan verlos
    //     en un mapa si se les reasigna el cliente).
    $tieneMun = columnExists($db, 'tblclientes', 'id_municipio');
    $tieneDoc = columnExists($db, 'tblclientes', 'id_documento');
    $tieneTermino = columnExists($db, 'tblclientes', 'Termino');
    $tieneWpp = columnExists($db, 'tblclientes', 'Whatsapp');
    $tieneLat = columnExists($db, 'tblclientes', 'latitud');
    $tieneLng = columnExists($db, 'tblclientes', 'longitud');
    $tienePrec= columnExists($db, 'tblclientes', 'precision_gps_metros');
    $tieneGpsAt = columnExists($db, 'tblclientes', 'gps_capturado_at');

    $cols = ['CodigoClien', 'Razon_Social', 'Nit', 'Telefonos', 'Email', 'Direccion'];
    if ($tieneWpp) $cols[] = 'Whatsapp';
    if ($tieneDoc) $cols[] = 'id_documento';
    if ($tieneMun) $cols[] = 'id_municipio';
    if ($tieneTermino) $cols[] = 'Termino';
    if ($tieneLat) $cols[] = 'latitud';
    if ($tieneLng) $cols[] = 'longitud';
    if ($tienePrec) $cols[] = 'precision_gps_metros';
    if ($tieneGpsAt) $cols[] = 'gps_capturado_at';

    $rows = $db->query("SELECT " . implode(', ', $cols) . " FROM tblclientes ORDER BY CodigoClien")
               ->fetchAll(PDO::FETCH_ASSOC);
    $registros = array_map(fn($r) => [
        'codigo'        => (string)$r['CodigoClien'],
        'razon_social'  => (string)($r['Razon_Social'] ?? ''),
        'nit'           => $r['Nit'] ?? null,
        'id_documento'  => isset($r['id_documento']) && $r['id_documento'] !== null ? intval($r['id_documento']) : null,
        'telefonos'     => $r['Telefonos'] ?? null,
        'celular'       => $r['Whatsapp']  ?? null,
        'email_cliente' => $r['Email']     ?? null,
        'direccion'     => $r['Direccion'] ?? null,
        'id_municipio'  => isset($r['id_municipio']) && $r['id_municipio'] !== null ? intval($r['id_municipio']) : null,
        'termino'       => isset($r['Termino']) && $r['Termino'] !== null ? intval($r['Termino']) : 0,
        'latitud'       => isset($r['latitud']) && $r['latitud'] !== null && $r['latitud'] !== '' ? floatval($r['latitud']) : null,
        'longitud'      => isset($r['longitud']) && $r['longitud'] !== null && $r['longitud'] !== '' ? floatval($r['longitud']) : null,
        'precision_gps_metros' => isset($r['precision_gps_metros']) && $r['precision_gps_metros'] !== null ? intval($r['precision_gps_metros']) : null,
        'gps_capturado_at'     => $r['gps_capturado_at'] ?? null,
    ], $rows);
    ...
    if ($db->query("SHOW TABLES LIKE 'tbl_vendedores_movil'")->fetch()) {
        $rows = $db->query("
            SELECT id, codigo, nombre, email, password_hash, telefono, cedula, zona,
                   can_edit_clients, activo
            FROM tbl_vendedores_movil
            WHERE activo = 1
        ")->fetchAll(PDO::FETCH_ASSOC);
        $registros = array_map(fn($r) => [
            // Lumen espera: id_vendedor_conta (obligatorio), codigo, nombre,
            // email_vendedor, password_hash. El id local es la ancla del upsert.
            'id_vendedor_conta' => intval($r['id']),
            'codigo'            => (string)$r['codigo'],
            'nombre'            => (string)$r['nombre'],
        ...
        // Marcar los sincronizados con éxito para que la UI del desktop lo vea
        if (($resultado['secciones']['vendedores']['fallas_http'] ?? 0) === 0 && count($rows) > 0) {
            $db->exec("UPDATE tbl_vendedores_movil SET sincronizado = 1 WHERE activo = 1");
        }
    }
