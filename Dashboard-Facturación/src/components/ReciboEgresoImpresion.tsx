...
export function ReciboEgresoImpresion({ egreso, formato, onClose }: Props) {
  ...
  const imprimir = () => {
    ...
    // Toolbar con botón manual — antes usaba window.onload -> print() -> close()
    // automático pero en impresoras térmicas o de red lentas el close() corría
    // antes del diálogo y salía hoja en blanco. Ahora el usuario controla.
    ...
    if (!win) return;
    const titulo = `Comprobante Egreso #${egreso.N_Comprobante}`;
    win.document.write(`
      <!DOCTYPE html>
