...
import { Search, RefreshCw, Plus, Ban, X, Printer, Edit2, DollarSign, TrendingDown, Tags } from 'lucide-react';
...
// Mismo tema que Inventario/StockBajo/HistorialCajas/ListadoPagos.
    ...
    <div style={{ padding: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: '#1f2937' }}>Gastos</h2>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => setShowNuevo(true)}
            style={{ height: 28, padding: '0 12px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 10 }}>
        {[
          { label: 'Total Gastos', value: String(resumen.total_gastos || 0), color: '#7c3aed', bg: '#f3e8ff', Ico: Tags },
          { label: 'Total Valor', value: fmtMon(resumen.total_valor || 0), color: '#991b1b', bg: '#fee2e2', Ico: DollarSign },
        ].map((s, i) => (
      ...
      {/* Resumen por categoría (pills compactos) */}
      {Object.keys(porCategoria).length > 0 && (
        <div style={{ display: 'flex', gap: 5, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginRight: 2 }}>Por categoría:</span>
          {Object.entries(porCategoria).sort((a, b) => b[1] - a[1]).map(([cat, val]) => (
            <div key={cat} style={{
              display: 'inline-flex', alignItems: 'center', gap: 5,
              padding: '3px 10px', borderRadius: 12, background: '#fff',
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input type="text" placeholder="Buscar gasto..." value={busqueda} onChange={e => setBusqueda(e.target.value)}
      ...
      <div style={{ height: 'calc(100vh - 300px)', minHeight: 300, width: '100%' }}>
        <AgGridReact ref={gridRef}
          ...
          overlayNoRowsTemplate="<span style='padding:20px;color:#6b7280'>Sin gastos en el período</span>"
        />
      </div>
